...
## 초기 구상 (2026-06 시작 메모 — 기록용. 일부는 만들면서 피벗됨)
- **문제**: Spotify 데스크탑 볼륨 곡선이 상단(80~100) 쏠림 = 위쪽 과민, 아래쪽 죽음 — ✅ 그대로
- **제약**: 무손실(Lossless, ≥1.2.84) 양보 불가 → Spotify 자동업뎃 필수 → **Spicetify는 업뎃마다 깨져서 부적합** — ✅ 지금도 핵심 제약
- **방식**: ~~OS레벨(Windows Core Audio, 앱별 세션 볼륨) 외부 제어~~
  - 🔄 **피벗**: OS 세션 볼륨은 **폰·Connect에 동기화가 안 됨**. → **UI Automation으로 Spotify *자체* 볼륨 슬라이더를 직접 움직이는** 방식으로 변경(어디서나 동기화). NAudio는 "세션 100% 리셋 1회"용으로만 남음.
- **스택**: C#/.NET, 트레이앱 — ✅ (단, Core Audio 대신 **UIA**가 핵심이 됨)
- **배포**: GitHub Releases 바이너리 — ✅ / winget·scoop 은 아직 백로그
- **선호 곡선**: `gain = slider^p`
  - 🔄 **피벗**: 처음엔 "p ≈ 0.35~0.5 = 강하게~균형"으로 **방향을 거꾸로** 봤음. 실제론 Spotify가 top-heavy라 **p<1이 "펴주는"**(평탄하게) 방향. 최종 0.3/0.4/0.6/1.0, 라벨도 정직하게(리니어/고름/살짝쏠림/디폴트).
- 💡 **이 메모엔 없던 것**: 지금의 간판 기능 **"네이티브 볼륨 레일 위 오버레이"**는 초기 구상에 아예 없었고 나중에 추가됨.

## 핵심 기능 (초기 목표 — 전부 구현됨 ✅)
- [x] 지각 볼륨 곡선: `gain = position^p`, p 조절 가능
- [x] 프리셋 (약하게/균형/강하게 → **리니어/고름/살짝쏠림/디폴트**로 진화)
- [x] **라이브 프리뷰**: 드래그하면 곡선 그래프 실시간(체감 음량 기준) ← Spicetify엔 없던 차별점, 실현
- [x] 일반 사용자도 UI에서 즉시 조절 (파일편집·CLI 없이)

## 긍정 리스트 (데모에서 좋았던 것)
- ⭐ **가사 팝업** (floating, 다중 소스) — 네이티브 가사 있지만 "항상 위 떠다니는 창"이 차별점

## 미구현 아이디어 / 참고 (백로그 — Spicetify 생태계에서 본 것들, 우린 네이티브로)
...
- [x] **떠 있는 가사 창** (`NowPlaying`/`Lyrics`/`LyricsForm`) — SMTC(Windows 미디어 컨트롤)로 현재 곡 인식(스포티파이 API·패치 없음), **LRCLIB 한 줄 동기화 + Genius 폴백**(스포티파이/LRCLIB에 가사 없는 신곡 등). 항상 위에 떠서 플리 보면서도 가사 유지 = 네이티브 대비 차별점. TFM을 `net8.0-windows10.0.19041.0`로 올림(WinRT SMTC).
- [ ] 폰 원격(보류), 볼륨 프로파일 저장/호출
